...
main(int argc, char *argv[])
{
    ...
    if (list)
    {
        ...
        if (status != 0) { printf("Error creating regex\n"); exit (3); }

		if (exclude_list)
		{
			status = regcomp(&exclude_re, exclude_list, REG_ICASE|REG_EXTENDED|REG_NOSUB);
			if (status != 0) { printf("Error creating exclusion regex\n"); exit (3); }
		}
...
int parse_perfdata(char *oldperfdatap, struct ifStruct *oldperfdata, char *prefix)
{
    ...
    for ( word = strtok_r(oldperfdatap, " ", &last); word; word = strtok_r(NULL, " ", &last))
    {
...
#ifdef DEBUG
           if (interface) fprintf(stderr, "interface %s found\n", interface);
#endif
        ...
        valstr = strchr(word, '=');
        if (valstr) { value = strtoull(valstr + 1, NULL, 10); }
        var = strtok_r(word, "=", &last2);

        if (interface && var && valstr) set_value(oldperfdata, interface, var, value, valstr + 1);
