			...
			if (status == STAT_SUCCESS &&
				...
				for (vars = vars; vars; vars = vars->next_variable) {
					...
					if (vars->type == ASN_OCTET_STR) {
						...
						if (i) {
							MEMCPY(interfaces[count].alias, vars->val.string,
								   vars->val_len);
			...
			if (status == STAT_SUCCESS &&
				...
				for (vars = vars; vars; vars = vars->next_variable) {
					...
					if (vars->type == ASN_OCTET_STR) {
						int i = (int)vars->name[(vars->name_length - 1)];
						if (i) {
							MEMCPY(interfaces[count].name, vars->val.string,
								   vars->val_len);
				...
				if (count < ifNumber) {
					...
					if (lastifflag) {
						fprintf(
							stderr,
							"Device has %d interfaces but only has %d names\n",
							ifNumber, count);
					} else {
...
void parse_and_check_commandline(int argc, char **argv,
	...
	while ((opt = getopt_long(argc, argv,
							  "aAb:c:dDe:f:h:j:J:k:K:m:Np:P:r:R:s:t:u:x:?",
							  longopts, NULL)) != -1) {
		switch (opt) {
		case 'a':
			...
			break;
		case 'A':
			config->get_aliases_flag =
				true; /* we need to see what we have matched... */
	...
	int status;
	if (config->list) {
		status = regcomp(&config->re, config->list,
